...
def fetch_dataloader(args, rank=0, world_size=1, use_ddp=False):
    ...
    if args.dataset == 'chairs':
        ...
        train_dataset = Spring(aug_params, split='train')
        ...
        train_dataset = 100 * sintel_clean + 100 * sintel_final + 400 * kitti + 120 * hd1k + things

    if use_ddp:
        train_sampler = torch.utils.data.distributed.DistributedSampler(
            train_dataset, num_replicas=world_size, rank=rank)
        num_gpu = torch.cuda.device_count()
        # print(calc_num_workers())
        train_loader = data.DataLoader(train_dataset, batch_size=args.batch_size // num_gpu, pin_memory=True,
            shuffle=(train_sampler is None), num_workers=16, sampler=train_sampler)
    else:
