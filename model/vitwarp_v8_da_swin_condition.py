...
class ViTWarpV8(nn.Module):
    def __init__(self, args):
        ...
        self.num_levels = 4
        self.num_radius = 4
        self.refine_net = SwinRefineNet(input_dim=self.network_dim*2)
        ...
        self.warp_linear = nn.Conv2d(2*self.network_dim+self.pretrain_dim+2, self.network_dim*2, 1, 1, 0, bias=True)
    ...
    def forward(self, image1, image2, iters=None, flow_gt=None):
        ...
        da_feats1 = self.da_feature(image1)
        da_feats2 = self.da_feature(image2)
        fmap1_feats = self.fnet(torch.cat([da_feats1['out'], image1], dim=1))
        fmap2_feats = self.fnet(torch.cat([da_feats2['out'], image2], dim=1))
        da_feats1_2x = F.interpolate(da_feats1['out'], scale_factor=0.5, mode='bilinear', align_corners=True)
        da_feats2_2x = F.interpolate(da_feats2['out'], scale_factor=0.5, mode='bilinear', align_corners=True)
        fmap1_2x = self.fmap_conv(torch.cat([fmap1_feats[0], da_feats1_2x], dim=1))
        fmap2_2x = self.fmap_conv(torch.cat([fmap2_feats[0], da_feats2_2x], dim=1))
        ...
        for itr in range(iters):
            ...
            flow_1x = F.interpolate(flow_2x, scale_factor=2, mode='bilinear', align_corners=True) * 2.0
            coords2 = (coords_grid(N, H, W, device=image1.device) + flow_1x).detach()
            warp_image2 = bilinear_sampler(image2, coords2.permute(0, 2, 3, 1))
            warp_da_feats2_pure = self.da_feature(warp_image2)['out']
            warp_da_feats2_pure_2x = F.interpolate(warp_da_feats2_pure, scale_factor=0.5, mode='bilinear', align_corners=True)
            refine_inp = self.warp_linear(torch.cat([fmap1_2x, net, da_feats1_2x, warp_da_feats2_pure_2x, flow_2x], dim=1))
            refine_outs = self.refine_net(refine_inp)
            net = self.refine_transform(refine_outs['out'])
