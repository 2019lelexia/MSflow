...
class ViTWarpV8(nn.Module):
    def __init__(self, args):
        ...
        self.hidden_conv1 = nn.Conv2d(self.network_dim*2, self.network_dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.hidden_conv2 = nn.Conv2d(self.network_dim*2, self.network_dim, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def forward(self, image1, image2, iters=None, flow_gt=None):
        ...
        fmap1_feats = fmap1_feats[0]
        fmap2_feats = fmap2_feats[0]
        # fmap1_2x_fuse = self.fmap_conv(torch.cat([fmap1_feats, da_feature1_2x], dim=1))
        # fmap2_2x_fuse = self.fmap_conv(torch.cat([fmap2_feats, da_feature2_2x], dim=1))
        net_fmap = self.hidden_conv1(torch.cat([fmap1_feats, fmap2_feats], dim=1))
        net_dino = self.hidden_conv2(torch.cat([da_feature1_2x, da_feature2_2x], dim=1))
        net = net_fmap + net_dino
        ...
        for itr in range(iters):
            ...
            warp_fmap2_2x = bilinear_sampler(fmap2_feats, coords2.permute(0, 2, 3, 1))
            warp_da_feature2_2x = bilinear_sampler(da_feature2_2x, coords2.permute(0, 2, 3, 1))

            refine_inp = self.warp_linear(torch.cat([fmap1_feats, warp_fmap2_2x, da_feature1_2x, warp_da_feature2_2x, net, flow_2x], dim=1))
