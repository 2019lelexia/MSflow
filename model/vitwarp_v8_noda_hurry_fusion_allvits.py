...
class ViTWarpV8(nn.Module):
    def __init__(self, args):
        ...
        self.refine_net_4x = VisionTransformer(args.network_backbone, self.network_dim * 2, patch_size=8)
        self.refine_net_1x = VisionTransformer(args.network_backbone, self.network_dim // 2, patch_size=8)
        ...
        self.hidden_conv_4x = nn.Conv2d(self.network_dim * 4, self.network_dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.hidden_conv_1x = nn.Conv2d(self.network_dim, self.network_dim, kernel_size=1, stride=1, padding=0, bias=True)
        
        self.warp_linear = nn.Conv2d(2*self.network_dim+self.network_dim+2, self.network_dim, 1, 1, 0, bias=True)
        self.warp_linear_4x = nn.Conv2d(2*self.network_dim*2+self.network_dim+2, self.network_dim*2, 1, 1, 0, bias=True)
        self.warp_linear_1x = nn.Conv2d(2*self.network_dim//2+self.network_dim+2, self.network_dim // 2, 1, 1, 0, bias=True)
        self.refine_transform = nn.Conv2d(self.network_dim//2 + self.network_dim//2 + self.network_dim, self.network_dim, 1, 1, 0, bias=True)
        self.refine_transform_4x = nn.Conv2d(self.network_dim//2 + self.network_dim, self.network_dim, 1, 1, 0, bias=True)
        self.refine_transform_1x = nn.Conv2d(self.network_dim//2 + self.network_dim //2 + self.network_dim//2 + self.network_dim, self.network_dim, 1, 1, 0, bias=True)
    ...
    def forward(self, image1, image2, iters=None, flow_gt=None):
        ...
        net_2x = self.hidden_conv(torch.cat([fmap1_2x, fmap2_2x], dim=1))
        net_4x = self.hidden_conv_4x(torch.cat([fmap1_4x, fmap2_4x], dim=1))
        net_1x = self.hidden_conv_1x(torch.cat([fmap1_1x, fmap2_1x], dim=1))
        ...
        for itr in range(iters):
            ...
            flow_2x_guide = F.interpolate(flow_1x, scale_factor=0.5, mode='bilinear', align_corners=True) * 0.5
            coords2_2x = (coords_grid(N, H // 2, W // 2, device=image1.device) + flow_2x_guide).detach()
            warp_2x = bilinear_sampler(fmap2_2x, coords2_2x.permute(0, 2, 3, 1))

            flow_4x_guide = F.interpolate(flow_1x, scale_factor=0.25, mode='bilinear', align_corners=True) * 0.25
            ...
            refine_inp_1x = self.warp_linear_1x(torch.cat([fmap1_1x, warp_1x, net_1x, flow_1x], dim=1))
            refine_inp_2x = self.warp_linear(torch.cat([fmap1_2x, warp_2x, net_2x, flow_2x_guide], dim=1))
            refine_inp_4x = self.warp_linear_4x(torch.cat([fmap1_4x, warp_4x, net_4x, flow_4x_guide], dim=1))

            refine_out_4x = self.refine_net_4x(refine_inp_4x)['out']
            refine_out_2x = self.refine_net(refine_inp_2x)['out']
            refine_out_1x = self.refine_net_1x(refine_inp_1x)['out']

            net_4x = self.refine_transform_4x(torch.cat([refine_out_4x, net_4x], dim=1))
            refine_out_4x_2x = F.interpolate(refine_out_4x, scale_factor=2, mode='bilinear', align_corners=True)
            net_2x = self.refine_transform(torch.cat([refine_out_2x, refine_out_4x_2x, net_2x], dim=1))
            refine_out_4x_1x = F.interpolate(refine_out_4x_2x, scale_factor=2, mode='bilinear', align_corners=True)
            refine_out_2x_1x = F.interpolate(refine_out_2x, scale_factor=2, mode='bilinear', align_corners=True)
            net_1x = self.refine_transform_1x(torch.cat([refine_out_1x, refine_out_4x_1x, refine_out_2x_1x, net_1x], dim=1))

            flow_update = self.flow_head_1x(net_1x)
            flow_1x = flow_1x + flow_update[:, :2]
            info_1x = flow_update[:, 2:]
