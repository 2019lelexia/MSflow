...
from model.backbone.vit import VisionTransformer, MODEL_CONFIGS, EfficientViTL1
...
class ViTWarpV8(nn.Module):
    def __init__(self, args):
        ...
        self.refine_net = EfficientViTL1(input_dim=self.network_dim)
        self.refine_net_4x = EfficientViTL1(input_dim=self.network_dim * 2)
        self.refine_net_1x = EfficientViTL1(input_dim=self.network_dim // 2)
        ...
        self.flow_head_4x = nn.Sequential(
            ...
            nn.Conv2d(self.network_dim * 2, 4*self.network_dim, 3, padding=1, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(4*self.network_dim, 6, 1, padding=0, bias=True)
        )
