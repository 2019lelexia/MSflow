...
class ViTWarpV8(nn.Module):
    ...
    def forward(self, image1, image2, iters=None, flow_gt=None):
        ...
        mae_feature1 = self.mae_feature(image1)['out']
        mae_feature1_2x = F.interpolate(mae_feature1, scale_factor=0.5, mode='bilinear', align_corners=True)
        ...
        for itr in range(iters):
            ...
            warp_mae_feature2 = self.mae_feature(warp_image2)['out']
            warp_mae_feature2_2x = F.interpolate(warp_mae_feature2, scale_factor=0.5, mode='bilinear', align_corners=True)
            refine_inp = self.warp_linear(torch.cat([fmap1_2x, fmap2_2x, mae_feature1_2x, warp_mae_feature2_2x, flow_2x], dim=1))
            refine_outs = self.refine_net(refine_inp)
            net_tmp = self.refine_transform(refine_outs['out'])
            flow_update = self.flow_head(net_tmp)
            weight_update = .25 * self.upsample_weight(net_tmp)
