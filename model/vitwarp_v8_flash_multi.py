...
class ViTWarpV8(nn.Module):
    def __init__(self, args):
        ...
        self.network_dim_b = MODEL_CONFIGS['vitb']['features']
        ...
        self.refine_net_b = VisionTransformer('vitb', self.network_dim_b, patch_size=8)
        ...
        self.fmap_conv_b = nn.Conv2d(self.pretrain_dim//2 + 128, self.network_dim_b, kernel_size=1, stride=1, padding=0, bias=True)
        ...
        self.hidden_conv_b = nn.Conv2d(self.network_dim_b*2, self.network_dim_b, kernel_size=1, stride=1, padding=0, bias=True)
        ...
        self.warp_linear_b = nn.Conv2d(3*self.network_dim_b+2, self.network_dim_b, 1, 1, 0, bias=True)
        ...
        self.refine_transform_b = nn.Conv2d(self.network_dim_b//2*3, self.network_dim_b, 1, 1, 0, bias=True)
        self.refine_transform_l = nn.Conv2d(self.network_dim_l//2*3, self.network_dim_l, 1, 1, 0, bias=True)
        self.fusion_block1 = HybridFusion(
            d_model=self.network_dim_b,
            context_dim=self.network_dim_l,
            num_blocks=1,
            num_heads=1
        )
        self.fusion_block2 = HybridFusion(
            d_model=self.network_dim,
            context_dim=self.network_dim_b,
            num_blocks=1,
            num_heads=1
        )
        self.fusion_block = nn.Conv2d(self.network_dim + self.network_dim_b + self.network_dim_l, self.network_dim, 1, 1, 0)
    ...
    def forward(self, image1, image2, iters=None, flow_gt=None):
        ...
        da_feature1_4x = F.interpolate(da_feature1['out'], scale_factor=0.25, mode='bilinear', align_corners=True)
        da_feature2_4x = F.interpolate(da_feature2['out'], scale_factor=0.25, mode='bilinear', align_corners=True)
        ...
        fmap1_4x = self.fmap_conv_b(torch.cat([fmap1_feats[1], da_feature1_4x], dim=1))
        fmap2_4x = self.fmap_conv_b(torch.cat([fmap2_feats[1], da_feature2_4x], dim=1))
        net_4x = self.hidden_conv_b(torch.cat([fmap1_4x, fmap2_4x], dim=1))
        ...
        for itr in range(iters):
            ...
            coords2_4x_guide = (coords_grid(N, H//4, W//4, device=image1.device) + flow_4x_guide)
            warp_4x = bilinear_sampler(fmap2_4x, coords2_4x_guide.permute(0, 2, 3, 1))
            ...
            refine_inp_b = self.warp_linear_b(torch.cat([fmap1_4x, warp_4x, net_4x, flow_4x_guide], dim=1))
            refine_outs_b = self.refine_net_b(refine_inp_b)
            ...
            net_4x = self.refine_transform_b(torch.cat([refine_outs_b['out'], net_4x], dim=1))
            net_8x = self.refine_transform_l(torch.cat([refine_outs_l['out'], net_8x], dim=1))
            ###### fusion
            net_4x = self.fusion_block1(net_4x, net_8x)
            net_fused = self.fusion_block2(net, net_4x)
            # net_4x_2x = F.interpolate(net_4x, scale_factor=2, mode='bilinear', align_corners=True)
            # net_8x_2x = F.interpolate(net_8x, scale_factor=4, mode='bilinear', align_corners=True)
            # net_fused = self.fusion_block(torch.cat([net, net_4x_2x, net_8x_2x], dim=1))
            net = net_fused
