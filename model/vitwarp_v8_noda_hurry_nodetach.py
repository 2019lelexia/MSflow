...
class ResNet18Deconv_0(nn.Module):
    def __init__(self, inp, oup):
        super(ResNet18Deconv_0, self).__init__()
...
class ViTWarpV8(nn.Module):
    def __init__(self, args):
        ...
        self.fnet = ResNet18Deconv_0(3, 64)
        ...
        self.fmap_conv_1x = nn.Conv2d(32, self.network_dim // 2, kernel_size=1, stride=1, padding=0, bias=True)
        ...
        self.hidden_conv_1x = nn.Conv2d(self.network_dim, self.network_dim // 2, kernel_size=1, stride=1, padding=0, bias=True)
        
        self.warp_linear = nn.Conv2d(4*self.network_dim+2, self.network_dim, 1, 1, 0, bias=True)
        self.warp_linear_4x = nn.Conv2d(6*self.network_dim+2, self.network_dim*2, 1, 1, 0, bias=True)
        self.warp_linear_1x = nn.Conv2d(2*self.network_dim+2, self.network_dim // 2, 1, 1, 0, bias=True)
        self.refine_transform = nn.Conv2d(self.network_dim//2*3, self.network_dim, 1, 1, 0, bias=True)
        self.refine_transform_4x = nn.Conv2d(self.network_dim + self.network_dim * 2, self.network_dim * 2, 1, 1, 0, bias=True)
        self.refine_transform_1x = nn.Conv2d(self.network_dim//4*3, self.network_dim // 2, 1, 1, 0, bias=True)

        self.net_4_2 = nn.Conv2d(self.network_dim * 2, self.network_dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.net_2_1 = nn.Conv2d(self.network_dim, self.network_dim // 2, kernel_size=1, stride=1, padding=0)
        ...
        self.upsample_weight_4x = nn.Sequential(
            # convex combination of 3x3 patches
            nn.Conv2d(self.network_dim * 2, 4*self.network_dim, 3, padding=1, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(4*self.network_dim, 4*9, 1, padding=0, bias=True)
        )
        ...
        self.flow_head_4x = nn.Sequential(
            # flow(2) + weight(2) + log_b(2)
            nn.Conv2d(self.network_dim * 2, 4*self.network_dim, 3, padding=1, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(4*self.network_dim, 2, 1, padding=0, bias=True)
        )
        self.flow_head_1x = nn.Sequential(
            # flow(2) + weight(2) + log_b(2)
            nn.Conv2d(self.network_dim // 2, self.network_dim, 3, padding=1, bias=True),
            nn.ReLU(inplace=True),
    ...
    def forward(self, image1, image2, iters=None, flow_gt=None):
        ...
        fmap1_1x = self.fmap_conv_1x(fmap1_feats[-1])
        fmap2_1x = self.fmap_conv_1x(fmap2_feats[-1])

        flow_2x = torch.zeros(N, 2, H//2, W//2).to(image1.device)
        flow_4x = torch.zeros(N, 2, H//4, W//4).to(image1.device)
        flow_1x = torch.zeros(N, 2, H, W).to(image1.device)

        hidden_state_4x = self.hidden_conv_4x(torch.cat([fmap1_4x, fmap2_4x], dim=1))
        refine_inp_4x = self.warp_linear_4x(torch.cat([fmap1_4x, fmap2_4x, hidden_state_4x, flow_4x], dim=1))
        refine_out_4x = self.refine_net_4x(refine_inp_4x)
        net_4x = self.refine_transform_4x(torch.cat([refine_out_4x['out'], hidden_state_4x], dim=1))
        flow_update_4x = self.flow_head_4x(net_4x)
        # weight_update_4x = .25 * self.upsample_weight_4x(net_4x)
        # flow_4x = flow_4x + flow_update_4x[:, :2]
        flow_4x = flow_4x + flow_update_4x
        # info_4x = flow_update_4x[:, 2:]

        # flow_4x_up_2x, info_4x_up_2x = self.upsample_data(flow_4x, info_4x, weight_update_4x)
        flow_4x_up_2x = F.interpolate(flow_4x, scale_factor=2, mode='bilinear', align_corners=True) * 2
        
        flow_2x = flow_2x + flow_4x_up_2x
        coords2_2x = (coords_grid(N, H//2, W//2, device=image1.device) + flow_2x)
        warp_2x = bilinear_sampler(fmap2_2x, coords2_2x.permute(0, 2, 3, 1))
        hidden_state_2x = self.hidden_conv(torch.cat([fmap1_2x, fmap2_2x], dim=1))
        net_2x = F.interpolate(net_4x, scale_factor=2, mode='bilinear', align_corners=True)
        net_2x = self.net_4_2(net_2x)
        refine_inp_2x = self.warp_linear(torch.cat([fmap1_2x, warp_2x, hidden_state_2x, net_2x, flow_2x], dim=1))
        refine_out_2x = self.refine_net(refine_inp_2x)
        net_2x = self.refine_transform(torch.cat([refine_out_2x['out'], hidden_state_2x], dim=1))
        flow_update_2x = self.flow_head(net_2x)
        # weight_update_2x = .25 * self.upsample_weight(net_2x)
        # flow_2x = flow_2x + flow_update_2x[:, :2]
        flow_2x = flow_2x + flow_update_2x
        # info_2x = flow_update_2x[:, 2:]

        # flow_2x_up_1x, info_2x_up_1x = self.upsample_data(flow_2x, info_2x, weight_update_2x)
        flow_2x_up_1x = F.interpolate(flow_2x, scale_factor=2, mode='bilinear', align_corners=True) * 2

        flow_1x = flow_1x + flow_2x_up_1x
        coords2_1x = (coords_grid(N, H, W, device=image1.device) + flow_1x)
        warp_1x = bilinear_sampler(fmap2_1x, coords2_1x.permute(0, 2, 3, 1))
        hidden_state_1x = self.hidden_conv_1x(torch.cat([fmap1_1x, fmap2_1x], dim=1))
        net_1x = F.interpolate(net_2x, scale_factor=2, mode='bilinear', align_corners=True)
        net_1x = self.net_2_1(net_1x)
        refine_inp_1x = self.warp_linear_1x(torch.cat([fmap1_1x, warp_1x, hidden_state_1x, net_1x, flow_1x], dim=1))
        refine_out_1x = self.refine_net_1x(refine_inp_1x)
        net_1x = self.refine_transform_1x(torch.cat([refine_out_1x['out'], hidden_state_1x], dim=1))
        flow_update_1x = self.flow_head_1x(net_1x)
        flow_1x = flow_1x + flow_update_1x[:, :2]
        info_1x = flow_update_1x[:, 2:]

        # flow_4x_up_1x, info_4x_up_1x = self.upsample_data(flow_4x_up_2x, info_4x_up_2x, weight_update_2x)

        # flow_predictions.append(flow_4x_up_1x)
        # flow_predictions.append(flow_2x_up_1x)
        flow_predictions.append(flow_1x)
        # info_predictions.append(info_4x_up_1x)
        # info_predictions.append(info_2x_up_1x)
