...
from model.backbone.depthanythingv2 import DepthAnythingFeature, ViTMAEFeature, ViTMAEFeature_nopos
from model.backbone.vit import SwinRefineNetDPT, MODEL_CONFIGS, SwinRefineNetDPT_nopos
...
class ViTWarpV8(nn.Module):
    def __init__(self, args):
        ...
        self.mae_feature = ViTMAEFeature_nopos()
        self.refine_net = SwinRefineNetDPT_nopos(input_dim=self.network_dim)
        ...
        self.fmap_conv_4x = nn.Conv2d(128 + self.pretrain_dim, self.network_dim*2, kernel_size=1, stride=1, padding=0, bias=True)
        self.fmap_conv_8x = nn.Conv2d(256 + self.pretrain_dim, self.network_dim*4, kernel_size=1, stride=1, padding=0, bias=True)
        ...
        self.refine_transform_4x = nn.Conv2d(self.network_dim + 128, 128, 1, 1, 0, bias=True)
        self.refine_transform_8x = nn.Conv2d(self.network_dim + 256, 256, 1, 1, 0, bias=True)
    ...
    def forward(self, image1, image2, iters=None, flow_gt=None):
        ...
        # initial feature
        mae_feature1 = self.mae_feature(image1)
        mae_feature2 = self.mae_feature(image2)
        ...
        mae_feature1_4x = F.interpolate(mae_feature1['path_1'], scale_factor=0.5, mode='bilinear', align_corners=True)
        mae_feature2_4x = F.interpolate(mae_feature2['path_1'], scale_factor=0.5, mode='bilinear', align_corners=True)
        mae_feature1_8x = F.interpolate(mae_feature1['path_2'], scale_factor=0.5, mode='bilinear', align_corners=True)
        mae_feature2_8x = F.interpolate(mae_feature2['path_2'], scale_factor=0.5, mode='bilinear', align_corners=True)
