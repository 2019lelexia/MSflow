...
class ViTWarpV8(nn.Module):
    def __init__(self, args):
        ...
        # self.warp_linear = nn.Conv2d(3*self.network_dim+81+2, self.network_dim, 1, 1, 0, bias=True)
        # self.warp_linear = nn.Conv2d(4*self.network_dim+1+2, self.network_dim, 1, 1, 0, bias=True)
        
        self.refine_transform = nn.Conv2d(self.network_dim//2*3, self.network_dim, 1, 1, 0, bias=True)
        # self.refine_transform = SepConvGRU(hidden_dim=self.network_dim, input_dim=self.network_dim // 2)
        self.fmap_conv_8x = nn.Conv2d(self.network_dim * 4, self.network_dim, 1, 1, 0)
        self.cv_dim = (self.num_levels * 2 + 1) ** 2 * self.num_levels
        self.cv_encoder = nn.Sequential(
            nn.Conv2d(self.cv_dim, self.network_dim, 1, 1, 0),
            nn.GELU(),
            nn.Conv2d(self.network_dim, self.network_dim, 3, 1, 1)
        )
        self.fusion_block = HybridFusion(dim=self.network_dim, num_blocks=1, num_heads=1, mlp_expand=2)
        self.fusion_final = nn.Conv2d(self.network_dim * 2, self.network_dim, 1, 1, 0)
    ...
    def forward(self, image1, image2, iters=None, flow_gt=None):
        ...
        corr = CorrBlock(fmap1_8x, fmap2_8x, num_levels=self.num_levels, radius=self.num_radius)
        ...
        for itr in range(iters):
            ...
            look_up_8x = corr(coords2_8x_guide)
            corr_feature_8x = self.cv_encoder(look_up_8x)
            q_in = fmap1_2x
            k_in = fmap1_8x
            v_in = corr_feature_8x
            fused_feature = self.fusion_block(q_in, k_in, v_in)
            refine_inp = self.fusion_final(torch.cat([refine_inp, fused_feature], dim=1))

            refine_outs = self.refine_net(refine_inp)
            net = self.refine_transform(torch.cat([refine_outs['out'], net], dim=1))
            # net = self.refine_transform(net, refine_outs['out'])
