...
from model.backbone.vit import SwinRefineNetDPT, MODEL_CONFIGS, SwinV2DPT
...
class ViTWarpV8(nn.Module):
    def __init__(self, args):
        ...
        self.pretrain_dim = 64
        
        self.network_dim = MODEL_CONFIGS[args.network_backbone]['features']
        self.fixed_feature = self.freeze_(SwinV2DPT(input_dim=3))
        ...
        self.warp_linear = nn.Conv2d(3*self.network_dim+2+self.pretrain_dim, self.network_dim, 1, 1, 0, bias=True)
    ...
    def forward(self, image1, image2, iters=None, flow_gt=None):
        ...
        # initial feature
        fixed_feature1 = self.fixed_feature(image1)
        fixed_feature2 = self.fixed_feature(image2)
        fmap1_feats = self.fnet(torch.cat([fixed_feature1['out'], image1], dim=1))
        fmap2_feats = self.fnet(torch.cat([fixed_feature2['out'], image2], dim=1))
        ...
        fixed_feature1_2x = F.interpolate(fixed_feature1['out'], scale_factor=0.5, mode='bilinear', align_corners=True)
        fixed_feature2_2x = F.interpolate(fixed_feature2['out'], scale_factor=0.5, mode='bilinear', align_corners=True)
        fmap1_2x = self.fmap_conv(torch.cat([fmap1_feats[0], fixed_feature1_2x], dim=1))
        fmap2_2x = self.fmap_conv(torch.cat([fmap2_feats[0], fixed_feature2_2x], dim=1))
        ...
        for itr in range(iters):
            ...
            if itr != 0:
                flow_1x_guide = F.interpolate(flow_2x, scale_factor=2, mode='bilinear', align_corners=True) * 2
                coords2_1x = (coords_grid(N, H, W, device=image1.device) + flow_1x_guide).detach()
                image2_warp_1x = bilinear_sampler(image2, coords2_1x.permute(0, 2, 3, 1))
                fixed_feature2_1x = self.fixed_feature(image2_warp_1x)
                fixed_feature2_2x = F.interpolate(fixed_feature2_1x['out'], scale_factor=0.5, mode='bilinear', align_corners=True)
            refine_inp = self.warp_linear(torch.cat([fmap1_2x, warp_2x, net, flow_2x, fixed_feature1_2x, fixed_feature2_2x], dim=1))
