...
from model.backbone.vit import MODEL_CONFIGS
...
class SingleHeadDeformableAttention(nn.Module):
    def __init__(self, dim, num_points=9, qkv_bias=True):
        """
        dim: 输入维度 (64)
        num_points: 采样点数，建议设为 9 (类似 3x3) 或 16 (类似 4x4)
        """
        super().__init__()
        self.num_points = num_points
        self.total_points = num_points # 单头，所以 total = num_points
        
        # 1. Q 和 V 的投影 (像 ViT 一样)
        self.to_q = nn.Linear(dim, dim, bias=qkv_bias)
        self.to_v = nn.Linear(dim, dim, bias=qkv_bias)

        # 2. 预测 Offset 和 Weight
        # 输入 Q，直接预测所有采样点的偏移和权重
        # 输出: 2 * num_points (xy偏移) + num_points (权重)
        self.to_offset_weights = nn.Linear(dim, 3 * self.num_points)
    ...
    def _reset_parameters(self):
        nn.init.constant_(self.to_offset_weights.weight, 0.)
        nn.init.constant_(self.to_offset_weights.bias, 0.)
        # 初始化 weights 部分，使其经过 Softmax 后均匀分布
        # 前 2*N 是 offset，后 N 是 weights
        with torch.no_grad():
            self.to_offset_weights.bias[2 * self.num_points:] = 0.0 
    ...
    def forward(self, x, flow=None):
        ...
        pred = self.to_offset_weights(q)
        
        offsets = pred[..., :2 * self.num_points] # [B, H, W, points*2]
        weights = pred[..., 2 * self.num_points:] # [B, H, W, points]
        ...
        grid = torch.stack((grid_x, grid_y), dim=-1).float().view(1, H, W, 1, 2).repeat(B, 1, 1, self.num_points, 1)

        if flow is not None:
            # flow: [B, 2, H, W] -> [B, H, W, 1, 2]
            flow_guide = flow.permute(0, 2, 3, 1).unsqueeze(3)
            grid = grid + flow_guide

        sampling_locations = grid + offsets.view(B, H, W, self.num_points, 2)
        sampling_locations_norm = sampling_locations.clone()
        sampling_locations_norm[..., 0] = 2 * (sampling_locations_norm[..., 0] / (W - 1)) - 1
        sampling_locations_norm[..., 1] = 2 * (sampling_locations_norm[..., 1] / (H - 1)) - 1
        sample_grid = sampling_locations_norm.view(B, H, W * self.num_points, 2)
        v_in = v.permute(0, 3, 1, 2)
        sampled_v = F.grid_sample(v_in, sample_grid, align_corners=True)
        sampled_v = sampled_v.view(B, C, H, W, self.num_points)
        weights = weights.unsqueeze(1)
        out = torch.sum(sampled_v * weights, dim=-1)
        out = out.permute(0, 2, 3, 1)
        return self.proj(out)

class ViTDeformableBlock(nn.Module):
    def __init__(self, dim, num_points=9, mlp_ratio=2.):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.attn = SingleHeadDeformableAttention(dim, num_points=num_points)
    ...
    def forward(self, x, flow=None):
        ...
        x_in = x_in + self.attn(self.norm1(x_in), flow=flow)
        x_in = x_in + self.mlp(self.norm2(x_in))
        return x_in.permute(0, 3, 1, 2)
...
class ViTWarpV8(nn.Module):
    def __init__(self, args):
        ...
        self.num_levels = 4
        self.num_radius = 4
        self.refine_net = ViTDeformableBlock(dim=self.network_dim, num_points=9)
