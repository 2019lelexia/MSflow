...
class ViTWarpV8(nn.Module):
    def __init__(self, args):
        ...
        self.fmap_conv_4x = nn.Conv2d(self.pretrain_dim + 128, 128, kernel_size=1, stride=1, padding=0, bias=True)
        self.fmap_conv_8x = nn.Conv2d(self.pretrain_dim + 256, 256, kernel_size=1, stride=1, padding=0, bias=True)
        ...
        self.warp_linear_4x = nn.Conv2d(3*128+2, 128, 1, 1, 0, bias=True)
        self.warp_linear_8x = nn.Conv2d(3*256+2, 256, 1, 1, 0, bias=True)

        self.refine_fusion = nn.Conv2d(self.network_dim + 128 + 256, self.network_dim, 1, 1, 0)
    ...
    def forward(self, image1, image2, iters=None, flow_gt=None):
        ...
        da_feature1_4x = F.interpolate(da_feature1['path_1'], scale_factor=0.5, mode='bilinear', align_corners=True)
        da_feature2_4x = F.interpolate(da_feature2['path_1'], scale_factor=0.5, mode='bilinear', align_corners=True)
        da_feature1_8x = F.interpolate(da_feature1['path_2'], scale_factor=0.5, mode='bilinear', align_corners=True)
        da_feature2_8x = F.interpolate(da_feature2['path_2'], scale_factor=0.5, mode='bilinear', align_corners=True)
        ...
        fmap1_4x = self.fmap_conv_4x(torch.cat([fmap1_feats[1], da_feature1_4x], dim=1))
        fmap2_4x = self.fmap_conv_4x(torch.cat([fmap2_feats[1], da_feature2_4x], dim=1))
        fmap1_8x = self.fmap_conv_8x(torch.cat([fmap1_feats[2], da_feature1_8x], dim=1))
        fmap2_8x = self.fmap_conv_8x(torch.cat([fmap2_feats[2], da_feature2_8x], dim=1))
