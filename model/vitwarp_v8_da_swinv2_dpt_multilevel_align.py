...
from model.backbone.vit import SwinV2DPT, MODEL_CONFIGS
...
class ViTWarpV8(nn.Module):
    def __init__(self, args):
        ...
        self.num_levels = 4
        self.num_radius = 4
        self.refine_net = SwinV2DPT(input_dim=self.network_dim * 3)
        ...
        self.fmap_conv_4x = nn.Conv2d(self.pretrain_dim//2 + 128, self.network_dim * 2, kernel_size=1, stride=1, padding=0, bias=True)
        ...
        self.refine_transform_4x = nn.Conv2d(self.network_dim//2*5, self.network_dim * 2, 1, 1, 0, bias=True)
    ...
    def forward(self, image1, image2, iters=None, flow_gt=None):
        ...
        padder = Padder(image1.shape, factor=64)
        ...
        for itr in range(iters):
            ...
            refine_outs = self.refine_net(torch.cat([refine_inp, refine_inp_4x_2x], dim=1))
            net = self.refine_transform(torch.cat([refine_outs['out'], net], dim=1))
            refine_outs_2x_4x = F.interpolate(refine_outs['out'], scale_factor=0.5, mode='bilinear', align_corners=True)
