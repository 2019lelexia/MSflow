...
class ViTWarpV8(nn.Module):
    def __init__(self, args):
        ...
        self.warp_linear_4x = nn.Conv2d(2*128+self.network_dim+2, 128, 1, 1, 0, bias=True)
        self.warp_linear_8x = nn.Conv2d(2*256+self.network_dim+2, 256, 1, 1, 0, bias=True)
    ...
    def forward(self, image1, image2, iters=None, flow_gt=None):
        ...
        for itr in range(iters):
            ...
            refine_inp = self.warp_linear(torch.cat([fmap1_2x, warp_2x, net, flow_2x], dim=1))
            net_4x = F.interpolate(net, scale_factor=0.5, mode='bilinear', align_corners=True)
            net_8x = F.interpolate(net, scale_factor=0.25, mode='bilinear', align_corners=True)
