...
from model.backbone.vit import TwinsRefineNet, MODEL_CONFIGS
...
class ViTWarpV8(nn.Module):
    def __init__(self, args):
        ...
        self.num_levels = 4
        self.num_radius = 4
        self.refine_net = TwinsRefineNet(input_dim=self.network_dim)
        ...
        self.fmap_conv = nn.Conv2d(32, self.network_dim, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def forward(self, image1, image2, iters=None, flow_gt=None):
        ...
        fmap1_1x = self.fmap_conv(fmap1_feats[-1])
        fmap2_1x = self.fmap_conv(fmap2_feats[-1])
        net = self.hidden_conv(torch.cat([fmap1_1x, fmap2_1x], dim=1))
        ...
        for itr in range(iters):
            ...
            refine_inp = self.warp_linear(torch.cat([fmap1_1x, warp_1x, net, flow_1x], dim=1))
