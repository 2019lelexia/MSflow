...
from model.backbone.depthanythingv2 import DepthAnythingFeature, ViTMAEFeature
from model.backbone.vit import SwinRefineNetDPT, MODEL_CONFIGS
...
class ViTWarpV8(nn.Module):
    def __init__(self, args):
        ...
        self.mae_feature = ViTMAEFeature()
        self.refine_net = SwinRefineNetDPT(input_dim=self.network_dim)
        ...
        self.warp_linear = nn.Conv2d(2*self.network_dim+2, self.network_dim, 1, 1, 0, bias=True)
        self.warp_linear_net = nn.Conv2d(2*self.network_dim+2, self.network_dim, 1, 1, 0, bias=True)
        self.warp_linear_4x = nn.Conv2d(2*self.network_dim*2+2, self.network_dim*2, 1, 1, 0, bias=True)
        self.warp_linear_8x = nn.Conv2d(2*self.network_dim*4+2, self.network_dim*4, 1, 1, 0, bias=True)
    ...
    def forward(self, image1, image2, iters=None, flow_gt=None):
        ...
        for itr in range(iters):
            ...
            refine_inp_nonet = self.warp_linear(torch.cat([fmap1_2x, warp_2x, flow_2x], dim=1))
            refine_inp_4x = self.warp_linear_4x(torch.cat([fmap1_4x, warp_4x, flow_4x_guide], dim=1))
            refine_inp_8x = self.warp_linear_8x(torch.cat([fmap1_8x, warp_8x, flow_8x_guide], dim=1))
            ...
            refine_inp_nonet = self.refine_fusion(torch.cat([refine_inp_nonet, refine_inp_4x_2x, refine_inp_8x_2x], dim=1))
            refine_inp = self.warp_linear_net(torch.cat([refine_inp_nonet, net, flow_2x], dim=1))
