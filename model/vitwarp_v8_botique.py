...
from model.backbone.depthanythingv2 import DepthAnythingFeature, ViTMAEFeature, ViTMAEFeature_abspos
from model.backbone.vit import SwinRefineNetDPT, MODEL_CONFIGS, SwinRefineNetDPT_abspos, VisionTransformer
...
class ViTWarpV8(nn.Module):
    def __init__(self, args):
        ...
        # self.mae_feature = ViTMAEFeature_abspos()
        # self.refine_net = SwinRefineNetDPT_abspos(input_dim=self.network_dim)
        self.refine_net = VisionTransformer(args.network_backbone, self.network_dim, patch_size=8)
        self.fnet = ResNet18Deconv(3, 64)
        # self.fnet = ResNet18Deconv(3, 64)
        self.fmap_conv = nn.Conv2d(64 + self.pretrain_dim//2, self.network_dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.fmap_conv_4x = nn.Conv2d(128 + self.pretrain_dim//2, self.network_dim*2, kernel_size=1, stride=1, padding=0, bias=True)
        self.fmap_conv_8x = nn.Conv2d(256 + self.pretrain_dim//2, self.network_dim*4, kernel_size=1, stride=1, padding=0, bias=True)
        ...
        self.hidden_conv_4x = nn.Conv2d(128*2, 128, kernel_size=1, stride=1, padding=0, bias=True)
        self.hidden_conv_8x = nn.Conv2d(256*2, 256, kernel_size=1, stride=1, padding=0, bias=True)

        self.warp_linear = nn.Conv2d(3*self.network_dim+2, self.network_dim, 1, 1, 0, bias=True)
        self.warp_linear_4x = nn.Conv2d(3*self.network_dim*2+2, self.network_dim*2, 1, 1, 0, bias=True)
        self.warp_linear_8x = nn.Conv2d(3*self.network_dim*4+2, self.network_dim*4, 1, 1, 0, bias=True)
        self.refine_fusion = nn.Conv2d(self.network_dim + self.network_dim*2 + self.network_dim*4, self.network_dim, 1, 1, 0)
        
        self.refine_transform = nn.Conv2d(self.network_dim//2*3, self.network_dim, 1, 1, 0, bias=True)
        self.refine_transform_4x = nn.Conv2d(self.network_dim//2 + 128, 128, 1, 1, 0, bias=True)
        self.refine_transform_8x = nn.Conv2d(self.network_dim//2 + 256, 256, 1, 1, 0, bias=True)
    ...
    def forward(self, image1, image2, iters=None, flow_gt=None):
        ...
        # initial feature
        mae_feature1 = self.da_feature(image1)
        mae_feature2 = self.da_feature(image2)
        ...
        mae_feature1_2x = F.interpolate(mae_feature1['out'], scale_factor=0.5, mode='bilinear', align_corners=True)
        mae_feature2_2x = F.interpolate(mae_feature2['out'], scale_factor=0.5, mode='bilinear', align_corners=True)
        mae_feature1_4x = F.interpolate(mae_feature1['out'], scale_factor=0.25, mode='bilinear', align_corners=True)
        mae_feature2_4x = F.interpolate(mae_feature2['out'], scale_factor=0.25, mode='bilinear', align_corners=True)
        mae_feature1_8x = F.interpolate(mae_feature1['out'], scale_factor=0.125, mode='bilinear', align_corners=True)
        mae_feature2_8x = F.interpolate(mae_feature2['out'], scale_factor=0.125, mode='bilinear', align_corners=True)

        fmap1_2x = self.fmap_conv(torch.cat([fmap1_feats[0], mae_feature1_2x], dim=1))
        fmap2_2x = self.fmap_conv(torch.cat([fmap2_feats[0], mae_feature2_2x], dim=1))
        fmap1_4x = self.fmap_conv_4x(torch.cat([fmap1_feats[1], mae_feature1_4x], dim=1))
        fmap2_4x = self.fmap_conv_4x(torch.cat([fmap2_feats[1], mae_feature2_4x], dim=1))
        fmap1_8x = self.fmap_conv_8x(torch.cat([fmap1_feats[2], mae_feature1_8x], dim=1))
        fmap2_8x = self.fmap_conv_8x(torch.cat([fmap2_feats[2], mae_feature2_8x], dim=1))

        net = self.hidden_conv(torch.cat([fmap1_2x, fmap2_2x], dim=1))
        net_4x = self.hidden_conv_4x(torch.cat([fmap1_4x, fmap2_4x], dim=1))
        net_8x = self.hidden_conv_8x(torch.cat([fmap1_8x, fmap2_8x], dim=1))
        ...
        for itr in range(iters):
            ...
            coords2_4x = (coords_grid(N, H//4, W//4, device=image1.device) + flow_4x_guide)
            ...
            coords2_8x = (coords_grid(N, H//8, W//8, device=image1.device) + flow_8x_guide)
            warp_8x = bilinear_sampler(fmap2_8x, coords2_8x.permute(0, 2, 3, 1))
            ...
            refine_inp_8x = self.warp_linear_8x(torch.cat([fmap1_8x, warp_8x, net_8x, flow_8x_guide], dim=1))
            refine_inp_4x_2x = F.interpolate(refine_inp_4x, scale_factor=2, mode='bilinear', align_corners=True)
            refine_inp_8x_2x = F.interpolate(refine_inp_8x, scale_factor=4, mode='bilinear', align_corners=True)
            refine_inp = self.refine_fusion(torch.cat([refine_inp, refine_inp_4x_2x, refine_inp_8x_2x], dim=1))

            # refine_inp = self.warp_linear(torch.cat([fmap1_2x, warp_2x, net, flow_2x], dim=1))
            refine_outs = self.refine_net(refine_inp)
            net = self.refine_transform(torch.cat([refine_outs['out'], net], dim=1))
            refine_outs_4x = F.interpolate(refine_outs['out'], scale_factor=0.5, mode='bilinear', align_corners=True)
            refine_outs_8x = F.interpolate(refine_outs['out'], scale_factor=0.25, mode='bilinear', align_corners=True)
            net_4x = self.refine_transform_4x(torch.cat([refine_outs_4x, net_4x], dim=1))
            net_8x = self.refine_transform_8x(torch.cat([refine_outs_8x, net_8x], dim=1))
