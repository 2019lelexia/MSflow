...
class ViTWarpV8(nn.Module):
    def __init__(self, args):
        ...
        self.occl_head = nn.Sequential(
            nn.Conv2d(self.network_dim, 2*self.network_dim, 3, padding=1, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(2*self.network_dim, 1, 1, padding=0, bias=True)
        )
        self.register_buffer("pos_weight", torch.tensor([20.0]))
        self.bce_loss_func = nn.BCEWithLogitsLoss(pos_weight=self.pos_weight, reduction='none')
    ...
    def upsample_data_with_occl(self, flow, info, occl, mask):
        N, C, H, W = info.shape
        mask = mask.view(N, 1, 9, 2, 2, H, W)
        mask = torch.softmax(mask, dim=2)
        ...
        up_info = F.unfold(info, [3, 3], padding=1)
        up_info = up_info.view(N, C, 9, 1, 1, H, W)
        up_occl = F.unfold(occl, [3, 3], padding=1)
        up_occl = up_occl.view(N, 1, 9, 1, 1, H, W)

        up_flow = torch.sum(mask * up_flow, dim=2)
        up_flow = up_flow.permute(0, 1, 4, 2, 5, 3)
        up_info = torch.sum(mask * up_info, dim=2)
        up_info = up_info.permute(0, 1, 4, 2, 5, 3)
        up_occl = torch.sum(mask * up_occl, dim=2)
        up_occl = up_occl.permute(0, 1, 4, 2, 5, 3)
        
        return up_flow.reshape(N, 2, 2*H, 2*W), up_info.reshape(N, C, 2*H, 2*W), up_occl.reshape(N, 1, 2*H, 2*W)
    ...
    def forward(self, image1, image2, iters=None, flow_gt=None, occl_gt=None):
        ...
        for itr in range(iters):
            ...
            occl_2x = self.occl_head(net)
            # upsample predictions
            # flow_up, info_up = self.upsample_data(flow_2x, info_2x, weight_update)
            flow_up, info_up, occl_up = self.upsample_data_with_occl(flow_2x, info_2x, occl_2x, weight_update)
        ...
        for i in range(len(info_predictions)):
            ...
            occl_predictions[i] = padder.unpad(occl_predictions[i])
        ...
        if flow_gt is not None:
            ...
            if occl_gt is not None:
                bce_predictions = []
                occl_gt = occl_gt.unsqueeze(1)
                for i in range(len(occl_predictions)):
                    bce_loss = self.bce_loss_func(occl_predictions[i], occl_gt)
                    bce_predictions.append(bce_loss)
                output = {'flow': flow_predictions, 'info': info_predictions, 'nf': nf_predictions, 'bce': bce_predictions}
            else:
