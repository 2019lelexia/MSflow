...
class DepthAnythingDepth(nn.Module):
    def __init__(self, encoder='vits', pretrained=True):
        super().__init__()
        self.model_configs = {
            'vitl': {'encoder': 'vitl', 'features': 256, 'out_channels': [256, 512, 1024, 1024]},
    ...
    def forward(self, x):
        ...
        out = self.depth_anything.depth_head.forward(features, patch_h, patch_w, return_intermediate=False)

        return {'out': out}  # out (depth) is 1
