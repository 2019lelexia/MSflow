...
class ViTWarpV8(nn.Module):
    def __init__(self, args):
        ...
        self.fmap_conv_4x = nn.Conv2d(128, 128, kernel_size=1, stride=1, padding=0, bias=True)
        self.fmap_conv_8x = nn.Conv2d(256, 256, kernel_size=1, stride=1, padding=0, bias=True)
        ...
        self.warp_linear_4x = nn.Conv2d(2*128+2, 128, 1, 1, 0, bias=True)
        self.warp_linear_8x = nn.Conv2d(2*256+2, 256, 1, 1, 0, bias=True)
        ...
        # self.fusion2 = HybridFusion(
        #     d_model=self.network_dim,
        #     context_dim=128,
        #     num_blocks=1,
        #     num_heads=1
        # )
        # self.fusion1 = HybridFusion(
        #     d_model=128,
        #     context_dim=256,
        #     num_blocks=1,
        #     num_heads=1
    ...
    def forward(self, image1, image2, iters=None, flow_gt=None):
        ...
        for itr in range(iters):
            ...
            refine_outs = self.refine_net(refine_inp)['out']
            net = self.refine_transform(torch.cat([refine_outs, net], dim=1))
