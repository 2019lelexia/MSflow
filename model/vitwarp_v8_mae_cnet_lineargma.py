...
from model.backbone.vit import SwinRefineNetDPT, MODEL_CONFIGS, vits_dino_16
...
class FastGMA(nn.Module):
    """
    基于线性注意力的全局运动聚合模块 (Linear Global Motion Aggregator)
    显存占用: O(N)
    """
    ...
    def forward(self, context_fmap, motion_fmap):
        heads = self.heads
        B, C, H, W = context_fmap.shape
        q, k = self.to_qk(context_fmap).chunk(2, dim=1)
        v = self.to_v(motion_fmap)
        q, k, v = map(lambda t: rearrange(t, 'b (h d) x y -> b h (x y) d', h=heads), (q, k, v))
        q = F.elu(q) + 1.0
        k = F.elu(k) + 1.0
        context_matrix = torch.einsum('b h n d, b h n e -> b h d e', k, v)
        sum_k = k.sum(dim=-2) 
        denom = torch.einsum('b h n d, b h d -> b h n', q, sum_k).unsqueeze(-1) + 1e-6
        out = torch.einsum('b h n d, b h d e -> b h n e', q, context_matrix)
        out = out / denom
        out = rearrange(out, 'b h (x y) d -> b (h d) x y', x=H, y=W)
        out = self.to_out(out)
        return motion_fmap + self.gamma * out
...
class ViTWarpV8(nn.Module):
    def __init__(self, args):
        ...
        self.num_levels = 4
        self.num_radius = 4
        self.refine_net = vits_dino_16(input_dim=self.network_dim)
        self.agg = FastGMA(dim=64, heads=1, dim_head=128)
        self.fnet = ResNet18Deconv(self.pretrain_dim//2 + 3, 64)
        self.cnet = ResNet18Deconv(self.pretrain_dim//2 + 3, 64)
        self.motion_conv = nn.Conv2d(self.network_dim // 2, self.network_dim, 1, 1, 0, bias=True)
        ...
        self.hidden_conv = nn.Conv2d(self.network_dim*3, self.network_dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.warp_linear = nn.Conv2d(3*self.network_dim+2, self.network_dim, 1, 1, 0, bias=True)
        self.refine_transform = nn.Conv2d(self.network_dim//2*5, self.network_dim, 1, 1, 0, bias=True)
    ...
    def forward(self, image1, image2, iters=None, flow_gt=None):
        ...
        fmap1_feats = self.fnet(torch.cat([mae_feature1['out'], image1], dim=1))
        fmap2_feats = self.fnet(torch.cat([mae_feature2['out'], image2], dim=1))
        ...
        cmap1_2x = self.cnet(torch.cat([mae_feature1['out'], image1], dim=1))[0]
        net = self.hidden_conv(torch.cat([fmap1_2x, fmap2_2x, cmap1_2x], dim=1))
        ...
        for itr in range(iters):
            ...
            refine_motion = self.motion_conv(refine_outs['out'])
            motion = self.agg(cmap1_2x, refine_motion)
            net = self.refine_transform(torch.cat([refine_outs['out'], net, motion], dim=1))
