...
def validate_sintel(model, cfg, rank=0):
    ...
    for dstype in ['clean', 'final']:
        ...
        for i_batch, data_blob in enumerate(val_loader):
            ...
            update_metrics(cfg, output, flow_gt, valid)
            # pbar.update(1)
        # pbar.close()
        print(f"Validation {dstype} EPE: {val_epe.avg}, 1px: {100 * (1 - val_px1.avg)}")
        results[dstype] = val_epe.avg
    return results

@torch.no_grad()
def validate_spring(model, cfg, rank=0):
    model.eval()
    results = {}
    reset_all_metrics()
    ...
    for i_batch, data_blob in enumerate(val_loader):
        ...
        # pbar.update(1)
    # pbar.close()
    print(f"Validation (Spring) EPE: {val_epe.avg}, 1px: {100 * (1 - val_px1.avg)}")
    results['spring_val'] = val_epe.avg
    return results

@torch.no_grad()
def validate_kitti(args, model, rank=0):
    ...
    return {'kitti-epe': val_epe.avg, 'kitti-f1': val_fl.avg}